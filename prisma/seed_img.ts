//  Attention, ici les photos sont envoyées dans bunny
//  il faudra gérer les images en local qui reste en local
//  et les avatars personnalisés sur bunny
//  ici ce n'est que pour le test
...
import {
  fileFromPath,
  processAvatarFile,
} from "@/lib/filesystem/img-processor";
...
async function main() {
  ...
  for (const user of users) {
    ...
    for (let i = 1; i < faker.number.int({ min: 10, max: 40 }); i++) {
      ...
      }); //dueDate between createdAt date and max 60 days more
